...
Compiler = require '../src/index'
compiler = new Compiler gfm: true, breaks: true, tasklist: true

describe 'text', ->

  it 'should be compiled', ->
    React.renderToStaticMarkup compiler.compile 'foo'
      .should.equal '<div><p>foo</p></div>'

describe 'escape', ->

  it 'should be compiled', ->
    React.renderToStaticMarkup compiler.compile '\\'
...
describe 'strong', ->
  ...
  it 'should be compiled when node has children', ->
    React.renderToStaticMarkup compiler.compile '**foo~~bar~~baz**'
...
describe 'link', ->
  ...
  it 'should be compiled', ->
    React.renderToStaticMarkup compiler.compile '[foo](http://example.com)'
      .should.equal '<div><p><a href="http://example.com">foo</a></p></div>'

  it 'should be compiled when node has children', ->
    React.renderToStaticMarkup compiler.compile '[foo**bar**baz](http://example.com)'
